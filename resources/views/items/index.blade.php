...
<div class="bg-white pb-4 px-4 rounded-md w-full">
    <div class="flex justify-end w-full pt-1 ">
        <a class="w-auto bg-purple-500 hover:bg-purple-700 rounded-lg shadow-xl font-medium text-white px-4 py-2"
            href="{{ route('items.create' )}}">Añadir Item</a>
    </div>
        ...
        <table class="table-auto border-collapse w-full">
            <thead>
                <tr class="rounded-lg text-sm font-medium text-gray-700 text-left" style="font-size: 0.9674rem">
                    ...
                    <th class="px-4 py-2 " style="background-color:#f8f8f8">NOMBRE</th>
                    <th class="px-4 py-2 " style="background-color:#f8f8f8">DESCRIPCION</th>
                    <th class="px-4 py-2 " style="background-color:#f8f8f8">IMAGEN</th>
                    <th class="px-4 py-2 " style="background-color:#f8f8f8">CATEGORIA LIGADA</th>
            ...
            <tbody class="text-sm font-normal text-gray-700">
                @foreach ($data as $key => $item)
                    <tr class="hover:bg-gray-100 border-b border-gray-200 py-10">
                        ...
                        <td class="px-4 py-4">{{ \Str::limit($item->description, 100) }}</td>
                        <td class="px-4 py-4"><img
                                class="w-auto h-14 rounded-sm border-gray-300 border transform hover:scale-125"
                                src="/storage/image/{{ $item->img }}"></td>
                        <td class="px-4 py-4">{{ $item->category_id }}</td>
                        <!--<td class="px-4 py-4">
                            @if ($item->status === true)
                        ...
                        <td class="px-4 py-4">
                            <div class="flex item-center justify-center">
                                <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                    <a class="btn btn-info" href="{{ route('items.show', $item->id) }}">
